...
Future<void> main(List<String> args) async {
  ...
  final tempDir = await Directory.systemTemp.createTemp(
    'beat_saber_song_toolkit_missing_install_',
  );
  final installDir = Directory(p.join(tempDir.path, 'CustomLevels'));
  ...
  try {
    ...
    print('installLimit=$limit');
    print('installCandidates=${missing.length}');
    print('installDir=${installDir.path}');
    ...
    for (final entry in missing) {
      try {
        ...
        final directory = await client.installLatestVersion(map, installDir);
        final scanned = await scanInstalledLibrary(installDir);
        final installedEntry = scanned
            .where((item) => item.mapId?.toLowerCase() == map.id.toLowerCase())
            .firstOrNull;
        if (installedEntry == null || !installedEntry.hasInfoDat) {
          throw StateError('Installed map not found after scan: ${map.id}');
        }
        installed += 1;
        installedIds.add(map.id);
        print(
          'installed id=${map.id} hash=${entry.hash.isEmpty ? '-' : entry.hash} '
          'directory=${directory.path} title=${installedEntry.title}',
        );
      } catch (error) {
        failed += 1;
        print(
    ...
    final finalScan = await scanInstalledLibrary(installDir);
    print('cacheFile=${cacheFile.path}');
    print('cacheEntries=${writtenCache.data.length}');
    print('installedIds=${installedIds.join(',')}');
    print('finalInstalledScan=${finalScan.length}');
    print('resolved=$resolved installed=$installed failed=$failed');
    if (missing.isEmpty) {
    ...
    if (installed == 0) {
      stderr.writeln('No missing playlist entry installed.');
      exitCode = 1;
      return;
    }
    print('playlistSyncMissingInstallSmoke=passed');
  } finally {
    if (keepTemp) {
...
const _usage = r'''
Usage:
  dart run tool\playlist_sync_missing_install_smoke.dart [options]
...
Options:
  ...
  --limit=N           Number of missing entries to install. Defaults to 1.
  --allow-network     Required. Confirms this run may contact BeatSaver.
  --keep-temp         Keep the temporary install/cache directory for inspection.
  --help, -h          Show this help.

Reads a real sample pack, finds playlist entries missing from the local folder,
resolves a small number through BeatSaver ID/hash lookups, installs them into a
temporary CustomLevels directory, and verifies scanInstalledLibrary sees them.
The real sample is read-only; no .bplist or real song directory is modified.
