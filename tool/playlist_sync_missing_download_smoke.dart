...
Future<void> main(List<String> args) async {
  ...
  final tempDir = await Directory.systemTemp.createTemp(
    'beat_saber_song_toolkit_missing_download_',
  );
  final downloadDir = Directory(p.join(tempDir.path, 'zips'));
  final cacheFile = File(p.join(tempDir.path, 'beatsaver_hash_cache.json'));
  final client = BeatSaverClient(
    requestTimeout: const Duration(seconds: 30),
    requestRetryCount: 1,
    downloadTimeout: const Duration(seconds: 60),
  ...
  try {
    ...
    print('downloadLimit=$limit');
    print('downloadCandidates=${missing.length}');
    ...
    for (final entry in missing) {
      try {
        ...
        final file = await client.downloadLatestVersion(map, downloadDir);
        final summary = await _inspectDownloadedZip(file);
        downloaded += 1;
        downloadedFiles.add(file.path);
        print(
          'downloaded id=${map.id} hash=${entry.hash.isEmpty ? '-' : entry.hash} '
          'bytes=${await file.length()} infoDat=${summary.hasInfoDat} '
          'audioFiles=${summary.audioFiles} file=${file.path}',
        );
      } catch (error) {
        failed += 1;
        print(
          'failed id=${entry.mapId.isEmpty ? '-' : entry.mapId} '
          'hash=${entry.hash.isEmpty ? '-' : entry.hash} error=$error',
        );
      }
    }
    ...
    print('downloadedFiles=${downloadedFiles.join(';')}');
    print('resolved=$resolved downloaded=$downloaded failed=$failed');
    if (missing.isEmpty) {
    ...
    if (downloaded == 0) {
      stderr.writeln('No missing playlist ZIP downloaded.');
      exitCode = 1;
      return;
    }
    print('playlistSyncMissingDownloadSmoke=passed');
  } finally {
    if (keepTemp) {
...
Future<_ZipInspection> _inspectDownloadedZip(File file) async {
  final archive = ZipDecoder().decodeBytes(await file.readAsBytes());
  final fileNames = archive.files
      .where((entry) => entry.isFile)
      .map((entry) => entry.name.replaceAll('\\', '/').toLowerCase())
      .toList(growable: false);
  final hasInfoDat = fileNames.any(
    (name) => p.basename(name).toLowerCase() == 'info.dat',
  );
  final audioFiles = fileNames
      .where(
        (name) =>
            name.endsWith('.egg') ||
            name.endsWith('.ogg') ||
      ...
      .length;
  if (!hasInfoDat) {
    throw FormatException('Downloaded ZIP has no Info.dat: ${file.path}');
  }
  if (audioFiles == 0) {
    throw FormatException('Downloaded ZIP has no audio file: ${file.path}');
  }
  return _ZipInspection(hasInfoDat: hasInfoDat, audioFiles: audioFiles);
}
...
class _ZipInspection {
  const _ZipInspection({required this.hasInfoDat, required this.audioFiles});
...
const _usage = r'''
Usage:
  dart run tool\playlist_sync_missing_download_smoke.dart [options]
...
Options:
  ...
  --limit=N           Number of missing entries to download. Defaults to 1.
  --allow-network     Required. Confirms this run may contact BeatSaver.
  --keep-temp         Keep the temporary ZIP/cache directory for inspection.
  --help, -h          Show this help.

Reads a real sample pack, finds playlist entries missing from the local folder,
resolves a small number through BeatSaver ID/hash lookups, downloads their ZIPs
to a system temp directory, and verifies the ZIP has Info.dat plus audio. The
real sample is read-only; no .bplist or song directory is modified.
